...
class Level3:
    ...
    def __init__(self):
        ...
        self.tiros = 2  # Número inicial de tiros
    ...
    def update(self):
        ...
        if Utils.verificar_colisao_retangulo(self.s[0], self.s[1], 570, (self.assets.y_luci-20), 50, 50):
            return 8  # Retorna 8 para indicar que o jogador perdeu
        
        if Utils.verificar_circulo_circulo(self.s[0], self.s[1], 10, self.assets.x_satelite, self.assets.y_satelite, self.assets.circle_radius) or Utils.verificar_circulo_circulo(self.s[0], self.s[1], 10, self.assets.x_sol, self.assets.y_sol, self.assets.circle_radius) or Utils.verificar_circulo_circulo(self.s[0], self.s[1], 10, self.assets.x_planeta, self.assets.y_planeta, self.assets.planet_radius):
            if self.tiros <= 0:
        ...
        if self.s[0] != 80 and self.s[1] != 190:
            # Calcula as forças gravitacionais
            ...
            gravitational_direction_3 = ((self.assets.x_planeta - self.assets.planet_radius, self.assets.y_planeta - self.assets.planet_radius) - self.s) / distance_3
            gravitational_force_3 = (self.assets.constante_planeta / distance_3**2) * gravitational_direction_3
            self.v = self.v + gravitational_force_3

        pg.display.update()

        return 7  # Retorna 7 para indicar que o jogo deve continuar
    ...
    def draw(self):
        ...
        # Desenha outros elementos na tela
        ...
        self.assets.screen.blit(self.assets.imagem_sol, (self.assets.x_sol - self.assets.circle_radius, self.assets.y_sol- self.assets.circle_radius))  # Desenha o sol
        self.assets.screen.blit(self.assets.imagem_planeta, ((self.assets.x_planeta - self.assets.planet_radius) -3.5, (self.assets.y_planeta - self.assets.planet_radius)-3.5))  # Desenha o planeta
